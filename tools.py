...
def compile(model,optimizer,loss,metrics):
  class LightningModel(pl.LightningModule):
    ...
    def prepare_input(self, x, y):
        x = ops.convert_to_tensor(x).type(torch.LongTensor).to(device)
        y = ops.convert_to_tensor(y).type(torch.LongTensor).to(device)
        return x, y
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(len(metrics_func)):
            self.log(f'train_{metrics_name[i]}', self.metrics_func[i](predictions, labels), prog_bar=True, logger=True)
        return loss
